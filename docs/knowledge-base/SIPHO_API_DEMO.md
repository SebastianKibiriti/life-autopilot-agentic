...
This is the hackathon-facing test for the supported consumer story. It uses
...
bakery location, evaluates every active commitment, checks the audit events,
re-evaluates with `student_has_started_moving=true`, and records a preparation
observation. The first evaluation should produce an actionable decision or a
safe escalation; the response exposes the reason, route provider, and whether
Gmail delivery succeeded. The event timeline proves the action was recorded.
